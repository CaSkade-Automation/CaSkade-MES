...
export class CapabilityService {
    ...
    async getAllCapabilities(): Promise<Array<CapabilityDto>> {
        try {
            ...
            const capabilities = converter.convertToDefinition(queryResult.results.bindings, capabilityMapping).capabilities as Array<CapabilityDto>;
            return capabilities;
        } catch (error) {
    ...
    async getCapabilityByIri(capabilityIri: string): Promise<CapabilityDto> {
        try {
            ...
            const capability = converter.convertToDefinition(queryResult.results.bindings, capabilityMapping).capabilities[0] as CapabilityDto;
            return capability;
        } catch (error) {
    ...
    async getCapabilitiesOfSkill(skillIri: string): Promise<CapabilityDto[]> {
        try {
            ...
            const capabilities = converter.convertToDefinition(queryResult.results.bindings, capabilityMapping).capabilities as CapabilityDto[];
            return capabilities;
        } catch (error) {
