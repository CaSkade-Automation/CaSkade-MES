...
export class MessageService {
    ...
    getAllMessages(): Observable<Message[]>{
        const obs= new Observable<Message[]>(observer => {
            ...
            //Modules  add
            this.moduleSocket.onModulesAdded().subscribe({
                next: (newModules) => {
                    newModules.forEach(module => {
                        this.addAndDeleteMessage(new Message("New Module",`Module "${module.getLocalName()}" added`, MessageType.Success));
                    });
                },
            ...
            // Modules change
            this.moduleSocket.onModulesChanged().subscribe({
            ...
            // Modules delete
            this.moduleSocket.onModuleDeleted().subscribe({
            ...
            // Skill added
            this.skillSocket.onSkillsAdded().subscribe({
                next: (newSkills) => {
                    newSkills.forEach(skill =>
                        this.addAndDeleteMessage(new Message("New Skill", `Skill "${skill.getLocalName()}" added`, MessageType.Success))
                    );
                },
            ...
            //Capabilities added
            this.capabilitySocket.onCapabilitiesAdded().subscribe({
                next: (newCapabilities) => {
                    newCapabilities.forEach(newCap =>
                        this.addAndDeleteMessage(new Message("New Capability",`Capability "${newCap.getLocalName()}" added`, MessageType.Success))
                    );
                },
                error: (err) => this.addAndDeleteMessage(new Message("Error during Capability registration",`Error: ${err}`, MessageType.Danger)),
            }),
            // Capabilities change
            this.capabilitySocket.onCapabilityChanged().subscribe({
            ...
            // Capabilities delete
            this.capabilitySocket.onCapabilityDeleted().subscribe({
